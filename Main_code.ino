...
SoftwareSerial DataUpload(10, 3);
SoftwareSerial VoiceControl(11, 4);
SoftwareSerial GSMControl(13, 6);
...
// ****** Battery operating relay panel *************

int BATRELAY_PIN1 = 23;
int BATRELAY_PIN2 = 25; 
int BATRELAY_PIN3 = 27;
int BATRELAY_PIN4 = 29;

int MAINRELAY_PIN1 = 22;
int MAINRELAY_PIN2 = 24;
int MAINRELAY_PIN3 = 26;
int MAINRELAY_PIN4 = 28;
...
Serial.begin(9600);
DataUpload.begin(2400);
VoiceControl.begin(9600);
GSMControl.begin(9600);
sensors.begin();
if (!sensors.getAddress(insideThermometer, 0))
sensors.setResolution(insideThermometer, 9); 
inputString = "";
inputGSM = "";
pinMode(MAINRELAY_PIN1,OUTPUT);
pinMode(MAINRELAY_PIN2,OUTPUT);
pinMode(MAINRELAY_PIN3,OUTPUT);
pinMode(MAINRELAY_PIN4,OUTPUT);

pinMode(BATRELAY_PIN1,OUTPUT);
pinMode(BATRELAY_PIN2,OUTPUT);
pinMode(BATRELAY_PIN3,OUTPUT);
pinMode(BATRELAY_PIN4,OUTPUT);

pinMode(EXRELAY_PIN1,OUTPUT);
pinMode(EXRELAY_PIN2,OUTPUT);
pinMode(EXRELAY_PIN3,OUTPUT);
pinMode(EXRELAY_PIN4,OUTPUT);
pinMode(EXRELAY_PIN5,OUTPUT);
pinMode(EXRELAY_PIN6,OUTPUT);
pinMode(EXRELAY_PIN7,OUTPUT);
pinMode(EXRELAY_PIN8,OUTPUT);
...
void loop() {
  ...
  // ************* Data upload system ***********
   ...
   while (DataUpload.available()>0) { 
   char inChar = (char)DataUpload.read();
   x= x+1;
   inputString += inChar;
   if (x==548){
    delay(1000);
  ...
  while (VoiceControl.available()){  
  delay(10); 
  char c = VoiceControl.read(); 
  if (c == '#') {break;}
  voice += c; 
  }
 ...
 //  GSM controller ***********************************
  ...
  while (GSMControl.available()>0){
    char inCharGSM = (char)GSMControl.read();
    inputGSM += inCharGSM;
    
    }

  if (inputGSM.length() > 0){
    GSMSearch();
 ...
 if (Temperature > Fire_temp.toInt()){
  FireResponse();
  }

 if (Temperature < Low_level_temp.toInt()){
  LowTempResponse();
  }

 if (Temperature == Custom1_temp.toInt()){
  Customtemp1Response();
  }

 if (Temperature == Custom2_temp.toInt()){
  Customtemp2Response();
  }
 // ****************************************************
...
void UploadDetails1(){
  ...
  char char_array[len];
  inputString.toCharArray(char_array, len); 
  for (int st=0;st<inputString.length();st++){
  int  y1= int(char_array[st]);
  EEPROM.write(st+10,y1);
  }
...
void VoiceSearch(){
  if(voice == star+Voice_controller_name + " " + Relay1_Name+ON){
   digitalWrite(MAINRELAY_PIN1,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Relay1_Name+OFF){
   digitalWrite(MAINRELAY_PIN1,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Relay2_Name+ON){
   digitalWrite(MAINRELAY_PIN2,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Relay2_Name+OFF){
   digitalWrite(MAINRELAY_PIN2,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Relay3_Name+ON){
   digitalWrite(MAINRELAY_PIN3,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Relay3_Name+OFF){
   digitalWrite(MAINRELAY_PIN3,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Relay4_Name+ON){
   digitalWrite(MAINRELAY_PIN4,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Relay4_Name+OFF){
   digitalWrite(MAINRELAY_PIN4,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay1_Name+ON){
   digitalWrite(EXRELAY_PIN1,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay1_Name+OFF){
   digitalWrite(EXRELAY_PIN1,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay2_Name+ON){
   digitalWrite(EXRELAY_PIN2,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay2_Name+OFF){
   digitalWrite(EXRELAY_PIN2,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay3_Name+ON){
   digitalWrite(EXRELAY_PIN3,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay3_Name+OFF){
   digitalWrite(EXRELAY_PIN3,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay4_Name+ON){
   digitalWrite(EXRELAY_PIN4,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay4_Name+OFF){
   digitalWrite(EXRELAY_PIN4,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay5_Name+ON){
   digitalWrite(EXRELAY_PIN5,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay5_Name+OFF){
   digitalWrite(EXRELAY_PIN5,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay6_Name+ON){
   digitalWrite(EXRELAY_PIN6,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay6_Name+OFF){
   digitalWrite(EXRELAY_PIN6,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay7_Name+ON){
   digitalWrite(EXRELAY_PIN7,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay7_Name+OFF){
   digitalWrite(EXRELAY_PIN7,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Ex_Relay8_Name+ON){
   digitalWrite(EXRELAY_PIN8,LOW); 
    }
  else if(voice == star+Voice_controller_name + " " + Ex_Relay8_Name+OFF){
   digitalWrite(EXRELAY_PIN8,HIGH); 
    }

  else if(voice == star+Voice_controller_name + " " + Power_OFF_code){
    if ((Voice_control_Instruction.substring(14, 15)).toInt()==1){
   digitalWrite(MAINRELAY_PIN1,HIGH); 
   digitalWrite(MAINRELAY_PIN2,HIGH); 
   digitalWrite(MAINRELAY_PIN3,HIGH); 
   digitalWrite(MAINRELAY_PIN4,HIGH); 
...
void UpdateBluetooth(){
  if ((Main_page_status.substring(2, 3)).toInt()==1){
 VoiceControl.begin(38400);
 delay(2000);
 digitalWrite(BATRELAY_PIN4,LOW);
 digitalWrite(BATRELAY_PIN3,LOW);
 delay(2000);
 digitalWrite(BATRELAY_PIN4,HIGH);
 delay(2000);
 VoiceControl.println("AT");    
 delay(1000);
 VoiceControl.println("AT");    
 delay(1000);
 VoiceControl.println("AT+NAME="+Bluetooth_Name);    //Sets the GSM Module in Text Mode
 delay(1000);
 VoiceControl.println("AT+PSWD="+Pairing_password);    //Sets the GSM Module in Text Mode
 delay(1000);
 VoiceControl.println("AT+ROLE="+ (Bluetooth_Instruction.substring(4, 5)).toInt());    //Sets the GSM Module in Text Mode
 delay(1000);
 VoiceControl.println("AT+CMODE="+ (Bluetooth_Instruction.substring(5, 6)).toInt());    //Sets the GSM Module in Text Mode
 delay(1000);
 VoiceControl.println("AT+UART="+Baud_rate+",0,0");    //Sets the GSM Module in Text Mode
 delay(1000);
 digitalWrite(BATRELAY_PIN4,LOW);
 digitalWrite(BATRELAY_PIN3,HIGH);
 delay(2000);
 digitalWrite(BATRELAY_PIN4,HIGH);
 VoiceControl.begin(9600);  
    }
...
void GSMSearch(){
  ...
  if (inputGSM.indexOf(Relay1_Name+ON) != -1){
    digitalWrite(MAINRELAY_PIN1,LOW); 
    inputGSM = "";
    }
  else if (inputGSM.indexOf(Relay1_Name+OFF) != -1){
    digitalWrite(MAINRELAY_PIN1,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Relay2_Name+ON) != -1){
    digitalWrite(MAINRELAY_PIN2,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Relay2_Name+OFF) != -1){
    digitalWrite(MAINRELAY_PIN2,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Relay3_Name+ON) != -1){
    digitalWrite(MAINRELAY_PIN3,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Relay3_Name+OFF) != -1){
    digitalWrite(MAINRELAY_PIN3,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Relay4_Name+ON) != -1){
    digitalWrite(MAINRELAY_PIN4,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Relay4_Name+OFF) != -1){
    digitalWrite(MAINRELAY_PIN4,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay1_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN1,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay1_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN1,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay2_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN2,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay2_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN2,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay3_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN3,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay3_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN3,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay4_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN4,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay4_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN4,HIGH); 
    inputGSM = "";
    }

else if (inputGSM.indexOf(Ex_Relay5_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN5,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay5_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN5,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay6_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN6,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay6_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN6,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay7_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN7,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay7_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN7,HIGH); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay8_Name+ON) != -1){
    digitalWrite(EXRELAY_PIN8,LOW); 
    inputGSM = "";
    }

 else if (inputGSM.indexOf(Ex_Relay8_Name+OFF) != -1){
    digitalWrite(EXRELAY_PIN8,HIGH); 
    inputGSM = "";
    }
...
void LowTempResponse(){
  
  
  }

void Customtemp1Response(){
  
  
  }

void Customtemp2Response(){
